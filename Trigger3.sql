...
BEGIN
	...
	FOR SELECT i.COD_PRODUTO, i.VALOR_VENDA FROM inserted i;
    -- Abrir o cursor
	OPEN iterator_valor_venda;
	FETCH NEXT FROM iterator_valor_venda INTO @cod_produto, @inserted_valor_venda;
...
BEGIN
    ...
    WHILE @@FETCH_STATUS = 0
	BEGIN
        IF @valor_venda IS NOT NULL AND
            NOT (@inserted_preco_base < ALL (
